...
<template>
  <div class="flex h-full min-h-0 flex-col shadow-sm">
    ...
    <div
      class="relative min-h-0 flex-1 overflow-hidden rounded-lg border border-slate-200 dark:border-zinc-600"
    >
            ...
            >
              <template v-for="childNode in node.children" :key="childNode.key">
                <div v-if="!props.filterFunction || props.filterFunction(childNode)" class="w-full">
                  ...
                  <div
                    v-if="childNode.expanded && childNode.children?.length"
                    class="overflow-hidden pl-4 transition-all duration-200 ease-in-out"
                  >
                    <div
                      v-for="grandchildNode in childNode.children"
                      :key="grandchildNode.key"
                      class="w-full"
                    >
                      <div
                        v-if="!props.filterFunction || props.filterFunction(grandchildNode)"
                        :class="[
                          'my-0.5 flex items-center rounded-md px-1 py-1.5 transition-colors duration-150 hover:bg-slate-100 dark:hover:bg-zinc-700',
                      ...
                      <div
                        v-if="grandchildNode.expanded && grandchildNode.children?.length"
                        class="overflow-hidden pl-4 transition-all duration-200 ease-in-out"
                      >
                        <template v-for="deepNode in grandchildNode.children" :key="deepNode.key">
                          ...
                          >
                            ...
                            <span
                              v-if="deepNode.children?.length"
                              class="ml-1 shrink-0 text-xs text-slate-400"
      ...
      >
        <div
          class="flex flex-col items-center gap-3 rounded-lg bg-white p-6 shadow-sm dark:bg-zinc-800"
        >
